#' Generate a MultiMAP analysis report template (Poisson endpoint)
#'
#' Writes a Quarto (.qmd) or R Markdown (.Rmd) report template for a Poisson
#' MultiMAP object (the output of \code{\link{SCI_pois}} or \code{\link{PI_pois}}),
#' suitable as a starting point for the statistical analysis plan (SAP) of a
#' study. The fitted object is saved next to the report as an \code{.rds} file;
#' the template loads it and rebuilds every table and figure with live code
#' chunks, so the document re-renders reproducibly. The italicised prose is
#' placeholder guidance for the user to replace.
#'
...
#' the concurrent control rate as a table and plot; and the robustified prior
#' (\code{$prior_rob}). \code{\link{SCI_pois}} output additionally gets an
#' analysis overview, the historical control data, the current trial, the
#' posterior distributions, the simultaneous credible intervals (table, plot and
#' interpretation) and a reference list. \code{\link{PI_pois}} output gets the
#' overview, the historical control data, the shared block and the reference list.
#'
...
#' \code{\link{SCI_pois}} / \code{\link{PI_pois}} as a named list), the template
...
#' and (for \code{SCI_pois} input) posterior -- followed by the non-borrowed
#' groups and the simultaneous credible intervals.
#'
#' @param x a Poisson MultiMAP object produced by \code{\link{SCI_pois}} or
#'   \code{\link{PI_pois}} (single-arm or multi-arm).
...
#' @examples
#' \donttest{
#' histdat <- data.frame(events = c(10, 13, 14, 15, 16, 17, 19, 20),
#'                       offset = rep(2, 8))
...
report_pois <- function(x,
        ...
        # Validate: Poisson MultiMAP object from SCI_pois() / PI_pois()
        ...
        # Output directory: default to a session temporary directory.
        if(is.null(path)){
                path <- tempdir()
        }
        ...
        #-----------------------------------------------------------------------
        # Body (identical for qmd / rmd). {{DATA_FILE}} is the only token substituted.
...
null_val <- if (identical(x$contr, "rate_diff")) 0 else 1
...
# Weight of the robust (non-informative) component, which robustify() appends as
# the last mixture component.
w_rob <- unname(x$prior_rob["w", ncol(x$prior_rob)])
```
)---"
...
group (CCG) of the current trial via a robustified prior for the control event
rate (Schmidli et al., 2014; Tarone, 1982), and compares each treatment group
...
historical control information as a robustified prior for the control event rate
(Schmidli et al., 2014; Tarone, 1982), derived with the RBesT package (Weber et
al., 2021), and reports the resulting `r 100 * conf`% prediction interval for the
rate of a future concurrent control group.*
)---"
...
```{r}
kable(x$histdat,
      caption = "Historical control studies (events, exposure/offset).")
```
)---"

        #--- current trial (SCI_pois() only) -----------------------------------
        current_trial <- r"---(
# Current trial
...
nd   <- x$newdat
show <- nd[, 1:3]
show$rate <- round(nd$mean, 3)
kable(show,
      caption = "Current trial: concurrent control (row 1) and treatment groups (events, exposure, observed rate).")
```
)---"
...
*The prior summarises the historical control information as a gamma mixture; it
is the informative prior for the concurrent control rate.*

```{r}
kable(mix_table(x$prior),
      caption = "Prior for the concurrent control rate (gamma mixture components).")
```

```{r}
#| fig-cap: "Prior density for the concurrent control rate (black: mixture; dashed: components)."
plot(as_mix(x$prior))
```

# Prediction interval

*The `r 100 * conf`% prediction interval for the rate of a concurrent control
group, derived from the prior-predictive (gamma-Poisson) distribution.*
...
kable(data.frame(median = round(unname(pint["median"]), 3),
      ...
      caption = sprintf("%.0f%% prediction interval for the concurrent control rate.", 100 * conf))
```

```{r}
#| fig-cap: "Gamma-Poisson predictive on the rate scale; bars inside the prediction interval are highlighted. For SCI_pois() input the observed concurrent control rate is drawn as a vertical line -- black if covered, red if not."
plot(x, which = 1)
```
...
kable(mix_table(x$prior_rob),
      caption = "Robustified prior for the concurrent control rate (gamma mixture components).")
```
...
```
        ...
        #--- posterior + simultaneous credible intervals (SCI_pois() only) -----
        posterior_sci <- r"---(
# Posterior distributions

## Concurrent control

*Posterior of the concurrent control rate, obtained by updating the robustified
...
kable(mix_table(x$posterior),
      caption = "Posterior of the concurrent control rate (gamma mixture components).")
```

```{r}
#| fig-cap: "Posterior density of the concurrent control rate."
plot(as_mix(x$posterior))
```

## Treatment groups

*Each treatment posterior is Gamma(eps + events, eps + exposure) with a near-flat
Gamma(eps, eps) prior updated with that arm's data; no historical information is
borrowed for the treatment arms.*

```{r}
trt <- x$newdat[-1, , drop = FALSE]
kable(data.frame(group     = trt[[1]],
                 post_a    = round(trt$posta, 3),
                 post_b    = round(trt$postb, 3),
                 post_mean = round(trt$posta / trt$postb, 3)),
      caption = "Treatment-group posteriors (gamma).")
```
...
Tarone, R. E. (1982). The use of historical control information in testing for a
trend in Poisson means. *Biometrics, 38*(2), 457--462.
https://doi.org/10.2307/2530459
...
# Fitted multi-arm object produced by SCI_pois() / PI_pois():
x <- readRDS("{{DATA_FILE}}")

# Render an RBesT gamma mixture (rows w, a, b) as a tidy table (mean = a / b).
mix_table <- function(m){
  cn <- colnames(m); if(is.null(cn)) cn <- paste0("comp", seq_len(ncol(m)))
  data.frame(component = cn,
             weight = round(m["w", ], 3),
...
the event rate (Schmidli et al., 2014; Tarone, 1982), updated with that arm's
current data. Treatment groups are compared using simultaneous credible intervals
at the `r 100 * conf`% level (Besag et al., 1995). The priors are derived with the
RBesT package (Weber et al., 2021).*
...
for the event rate (Schmidli et al., 2014; Tarone, 1982), derived with the RBesT
...
every other arm uses a non-borrowing near-flat Gamma(eps, eps) prior.*

```{r}
nd   <- x$newdat
show <- nd[, 1:3]
show$rate <- round(nd$mean, 3)
kable(show, caption = "Current trial groups (events, exposure, observed rate).")
```
)---"
...
```{r}
kable(x$histdat[["<<ARM>>"]],
      caption = "Historical control studies for arm <<ARM>> (events, offset).")
```
...
kable(mix_table(x$prior[["<<ARM>>"]]),
      caption = "Prior for arm <<ARM>> (gamma mixture components).")
```
...
kable(data.frame(median = round(unname(pint["median"]), 3),
                 lower  = round(unname(pint["lower"]),  3),
                 upper  = round(unname(pint["upper"]),  3)),
      caption = "Prediction interval for arm <<ARM>> (rate scale).")
```

```{r}
#| fig-cap: "Gamma-Poisson predictive for arm <<ARM>> on the rate scale."
plot(x, arm = "<<ARM>>")
```
...
kable(mix_table(x$posterior[["<<ARM>>"]]),
      caption = "Posterior for arm <<ARM>> (gamma mixture components).")
```
...
Gamma(eps + events, eps + exposure) with eps = 0.001.*
...
if (length(nb)) {
  ...
  pa  <- 0.001 + trt[[2]]; pb <- 0.001 + trt[[3]]
  kable(data.frame(group = trt[[1]], post_a = round(pa, 3), post_b = round(pb, 3),
                   post_mean = round(pa / pb, 3)),
        caption = "Non-borrowed groups: posteriors (gamma).")
}
```
)---"
